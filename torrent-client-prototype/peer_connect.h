...
public:
    ...
    bool IsPieceAvailable(size_t pieceIndex) const {
        size_t num_of_byte = pieceIndex / 8;
        if (num_of_byte >= bitfield_.size()) {
            //
            return false;
        }
        size_t num_of_bit = 7 - (pieceIndex % 8);
        return (bitfield_[num_of_byte] & (1 << num_of_bit));
    }
    ...
    void SetPieceAvailability(size_t pieceIndex) {
        size_t num_of_byte = pieceIndex / 8;
        if (num_of_byte >= bitfield_.size()) {
            bitfield_.resize(2 * num_of_byte, 0);
        }
        size_t  num_of_bit = 7 - (pieceIndex % 8);
        bitfield_[num_of_byte] = bitfield_[num_of_byte] | (1 << num_of_bit);
    }
    /*
...
private:
    ...
    bool pendingBlock_=false;  // уже послали запрос на скачивание части файла и ждем ответ
    bool failed_=false; // соединение не удалось установить или оно было разорвано в результате ошибки
    std::mutex mutex_;
    int count = 0;
